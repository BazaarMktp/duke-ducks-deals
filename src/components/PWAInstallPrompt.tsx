...
export const PWAInstallPrompt = () => {
  ...
  useEffect(() => {
    ...
    const handleAppInstalled = () => {
      ...
      toast({
        title: "App Installed!",
        description: "Devil's Marketplace has been added to your home screen.",
      });
    };
  ...
  const handleInstallClick = async () => {
    ...
    if (outcome === 'accepted') {
      toast({
        title: "Installing...",
        description: "Devil's Marketplace is being added to your home screen.",
      });
    }
    setDeferredPrompt(null);
    setShowPrompt(false);
  ...
  if (isSafariDevice && !deferredPrompt) {
    return (
      <Card className="fixed bottom-20 left-4 right-4 md:left-auto md:right-4 md:w-80 p-4 shadow-lg z-50 bg-card border-primary/20">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2">
            <Share className="h-5 w-5 text-primary" />
            <h3 className="font-semibold text-sm">Install Devil's Marketplace</h3>
          </div>
        ...
        <p className="text-sm text-muted-foreground mb-3">
          Get the full app experience! Install for faster access and offline features.
        </p>
        ...
        <Button variant="outline" onClick={handleDismiss} size="sm" className="w-full">
          Got it
        </Button>
      </Card>
    );
