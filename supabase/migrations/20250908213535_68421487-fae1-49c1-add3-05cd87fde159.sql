-- Create a table to track listing engagement metrics
CREATE TABLE IF NOT EXISTS public.listing_engagement (
...
-- Enable RLS
ALTER TABLE public.listing_engagement ENABLE ROW LEVEL SECURITY;

-- Create policies for listing engagement
CREATE POLICY "Anyone can view engagement metrics" 
ON public.listing_engagement 
FOR SELECT 
USING (true);
...
-- Create function to auto-feature desirable listings
CREATE OR REPLACE FUNCTION public.auto_feature_desirable_listings()
RETURNS INTEGER AS $$
DECLARE
  updated_count INTEGER := 0;
BEGIN
  -- First, unfuture all currently featured items
